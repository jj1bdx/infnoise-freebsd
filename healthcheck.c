...
The theory behind this is simple.  The next bit from the INM TRNG can be
guessed, based on the previous bits, by measuring how often a 0 or 1 occurs
given the previous bits.  Update these statistics dynamically, and use them to
determine how hard it would be to predict the current state.

For example, if 0100 is followed by 1 80% of the time, and we read a 1, the
probability of the input string being what it is decreases by multiplying it by
0.8.  If we read a 0, we multiply the likelyhood of the current state by 0.2.

Because INMs generate about log(K)/log(2) bits per clock when K is the gain used
in the INM (between 1 and 2), we know how much entropy there should be coming
from the device. If the measured entropy diverges too strongly from the
theoretical entropy, we should shut down the entropy source, since it is not
working correctly.

An assumption made is that bits far enough away are not correlated.  This is
directly confirmed.
...
void inmDumpStats(void) {
    ...
    for (i = 0u; i < 1u << inmN; i++) {
        printf("%x onesEven:%u zerosEven:%u onesOdd:%u zerosOdd:%u\n", i,
               inmOnesEven[i], inmZerosEven[i], inmOnesOdd[i], inmZerosOdd[i]);
    }
}
...
// Initialize the health check.  N is the number of bits used to predict the
// next bit. At least 8 bits must be used, and no more than 30.  In general, we
// should use bits large enough so that INM output will be uncorrelated with
// bits N samples back in time.
bool inmHealthCheckStart(uint8_t N, double K, struct opt_struct *opts) {
    ...
    resetStats();
    if (inmOnesEven == NULL || inmZerosEven == NULL || inmOnesOdd == NULL ||
        inmZerosOdd == NULL) {
        inmHealthCheckStop();
        return false;
    }
...
// If running continuously, it is possible to start overflowing the 32-bit
// counters for zeros and ones.  Check for this, and scale the stats if needed.
...
static void scaleZeroOneCounts(void) {
    uint64_t maxVal =
        inmTotalZeros >= inmTotalOnes ? inmTotalZeros : inmTotalOnes;
...
bool inmHealthCheckAddBit(bool evenBit, bool oddBit, bool even) {
    ...
    if (inmDebug && (inmTotalBits & 0xfffffllu) == 0u) {
        fprintf(stderr,
                "Generated %llu bits.  %s to use data.  Estimated entropy per "
                "bit: %f, estimated K: %f\n",
                (long long)inmTotalBits,
                inmHealthCheckOkToUseData() ? "OK" : "NOT OK",
        ...
        fprintf(stderr, "num1s:%f%%, even misfires:%f%%, odd misfires:%f%%\n",
                ...
                inmEvenMisfires * 100.0 / inmNumBitsSampled,
                inmOddMisfires * 100.0 / inmNumBitsSampled);
        fflush(stderr);
    }
...
// Return true if the health checker has enough data to verify proper operation
// of the INM.
...
uint32_t inmGetEntropyLevel(void) { return inmEntropyLevel; }

// Reduce the entropy level by numBytes.
void inmClearEntropyLevel(void) { inmEntropyLevel = 0u; }
...
// Compare the ability to predict with 1 fewer bits and see how much less
// accurate we are.
static void checkLSBStatsForNBits(uint8_t N) {
    ...
    printf("Probability of guessing correctly with %u bits: %f\n", N,
           (double)totalRight / totalGuesses);
}
