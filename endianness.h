...
/* Try to derive from arch/compiler-specific macros */
    ...
    (defined(_LITTLE_ENDIAN) && _LITTLE_ENDIAN == 1) || defined(_M_IX86) ||    \
...
#error                                                                         \
    "Sorry, we couldn't detect endiannes for your system! Please set -DENDIANNESS_LE=1 or 0 using your CPPFLAGS/CFLAGS and open an issue for your system on https://github.com/rofl0r/endianness.h - Thanks!"
#endif
...
static __inline uint64_t end_bswap64(uint64_t __x) {
    return ((end_bswap32((uint32_t)__x) + 0ULL) << 32) |
           (end_bswap32(__x >> 32));
}
